...
public class NotificationValidationService : INotificationValidationService
{
    ...
    public void Validate(NotificationDto notificationDto) 
    {
        ...
        if ((notificationDto.StartTime - notificationDto.EndTime) < _settings.MinInterval)
            throw new InvalidOperationException("Notification's {_settings.MinInterval} must be greater than difference between start time and end time");
        ...
        {
            if (day < 1 || day > 7)
                throw new InvalidOperationException("Day must be more than zero and less than seven");
        });
